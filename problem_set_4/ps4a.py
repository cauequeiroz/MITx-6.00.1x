...
def isValidWord(word, hand, wordList):
    ...
    """
    wordInWordList = word in wordList
    wordInHandDict = True
   
    wordFrequencyDict = dict()
    for letter in word:
        wordFrequencyDict[letter] = wordFrequencyDict.get(letter, 0) + 1
    
    for (letter, frequency) in wordFrequencyDict.items():
        if letter not in hand or hand[letter] < frequency:
            wordInHandDict = False
            break

    return wordInHandDict and wordInWordList        
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    print(message, 'Total score:', score)
